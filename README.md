...
A set of golden ratios are generated from the samples in the
...
is the z-score. It indicates how many standard deviations is the batch distance from the mean of the normal logs.
Higher the score higher the degree of abnormality.
...
### Visualizations
  ...
  the trained model against validation dataset is as below: ![distribution](./docs/images/cctrng_best.png "Distance Separation: Normal vs Anomalous")

  The above visualization can be generated by running the command:
  ```
  $ python src/main.py visualize-separation distribution
  ```

  The t-SNE plot showing visualizing the separation between the embeddings of anchor, positive, and negative samples is as below:
  ![embedding](./docs/images/tsne_embedding.png "t-SNE visualization of Embedding space")

  The above visualization can be generated by running the command:
  ```
  $ python src/main.py visualize-separation embedding
  ```
...
generate-dummy-logs    Generate synthetic logs. The logs will be generated under <project_root>/synthetic.
                       Three sets of logs: anchors_(train|valid).csv, positives_(train|valid).csv, and negatives_(train_[easy|medium|hard|extreme]|valid).csv
                       will be generated. Size of each batch will be 10 and total number of log lines will be 10 * batch_count.
train-model            Train the model using the training and validation log samples available under <project_root>/synthetic.
                       State of the trained model and other learned parameters will be saved under specified state_root_dir, if one is specified.
                       If no directory is specified the trained model state and related configuration will be saved under <project_root>/saved_states.
infer                  Run inference on the batch of logs contained in given file path. The inference will be run on batch of 10 logs at a time,
                       and classification result will be on each batch (of 10) logs. State of the trained model and other necessary parameter will be
                       loaded from specified state_root_dir, if one is specified. If no directory
                       is specified the trained model state and related parameters will be loaded from <project_root>/saved_states.
visualize-separation   Generate plots visualizing separation between normal and anomalous samples in the validation dataset.
